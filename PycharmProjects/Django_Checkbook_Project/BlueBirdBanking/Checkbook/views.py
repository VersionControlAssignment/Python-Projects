...
def balance(request, pk):
    ...
    for t in transactions: # Loop through transactions and determine which is a deposit or withdrawal
        if t.type == 'Deposit':
            current_total += t.amount  # IOf deposit add amount to balance
            ...
            table_contents.update({t: current_total}) # Add transaction and total to the dictionary
    # Pass account, account total balance, and transaction information to th template
...
def transaction(request):
    ...
    # Checks if request method is POST
    if request.method == 'POST':
        if form.is_valid(): # Saves the transaction form
